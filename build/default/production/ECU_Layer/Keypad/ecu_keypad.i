...
Std_ReturnType keypad_initialize(const keypad_t *kaypad_obj){
    ...
    if(((void*)0) == kaypad_obj)
    {
        ret = (Std_ReturnType)0x00;
    }
    else
    {
        uint8 rows_cnt = 0, cols_cnt = 0;
        for(rows_cnt = 0; rows_cnt < 4; rows_cnt++)
        {
            ret = gpio_pin_initialize(&(kaypad_obj->keypad_row_pins[rows_cnt]));
        }
        for(cols_cnt = 0; cols_cnt < 4; cols_cnt++)
        {
...
Std_ReturnType keypad_get_value(const keypad_t *kaypad_obj, uint8 *value){
    Std_ReturnType ret = (Std_ReturnType)0x01 ;
    if(((void*)0) == kaypad_obj || ((void*)0) == value)
    {
        ret = (Std_ReturnType)0x00;
    }
    else
    {
        uint8 current_colmn = 0, current_row = 0;
        uint8 l_row_cnt = 0, current_col_logic = 0;
        ...
        for(current_row = 0; current_row < 4; current_row++)
        {
            ...
            ret = gpio_pin_write_logic(&(kaypad_obj->keypad_row_pins[current_row]), GPIO_HIGH);
            for(current_colmn = 0; current_colmn < 4; current_colmn++)
            {
